...
const sanitize = (u) => ({
  id: u._id, name: u.name, email: u.email, studioName: u.studioName,
  phone: u.phone, profileImage: u.profileImage, studioLogo: u.studioLogo,
  document: u.document, isAdmin: u.isAdmin,
  subscription: {
    plan: u.subscription.plan, status: u.subscription.status,
    trialEndsAt: u.subscription.trialEndsAt, expiresAt: u.subscription.expiresAt,
  },
});
...
router.post('/register', async (req, res) => {
  ...
  if (!consentAcceptedAt) return res.status(400).json({ error: 'Aceite dos termos é obrigatório.' });

  const safeName   = String(name).trim().slice(0, 100);
  const safeEmail  = String(email).trim().toLowerCase().slice(0, 200);
  const safeStudio = String(studioName || '').trim().slice(0, 100);

  if (await User.findOne({ email: safeEmail })) return res.status(400).json({ error: 'Email já cadastrado.' });

  const passwordHash = await bcrypt.hash(password, 12);
  const user = await User.create({
    name: safeName, email: safeEmail, passwordHash, studioName: safeStudio,
    tokenVersion: 0, consentAcceptedAt: consentAcceptedAt || new Date(), consentVersion: '2.0',
  });
...
router.put('/password', auth, async (req, res) => {
  ...
  if (!current || !newPassword) return res.status(400).json({ error: 'Senha atual e nova senha são obrigatórias.' });
  if (newPassword.length < 6) return res.status(400).json({ error: 'Nova senha deve ter pelo menos 6 caracteres.' });

  const user = await User.findById(req.user._id);
  if (!(await bcrypt.compare(current, user.passwordHash))) return res.status(400).json({ error: 'Senha atual incorreta.' });
        ...
        <body style="margin:0;padding:0;background:#0a0a0a;font-family:Arial,sans-serif;">
          <div style="max-width:500px;margin:0 auto;padding:32px 20px;">
            <div style="background:#111;border:1px solid #222;border-radius:16px;padding:32px;text-align:center;">
              ...
              <a href="${link}" style="display:inline-block;background:linear-gradient(135deg,#E87722,#C85A00);color:#fff;padding:14px 32px;border-radius:10px;text-decoration:none;font-size:15px;font-weight:700;">
