...
router.post('/client/login', async (req, res) => {
  ...
  const photos = await Promise.all(gallery.photos.map(async p => ({
    id: p.id, filename: p.filename, selected: p.selected,
    thumbnailUrl: p.b2FileName ? await getSignedPhotoUrl(p.b2FileName.replace('.webp','_thumb.webp')).catch(()=>null) : null,
    // URL full só se download habilitado
    url: (gallery.downloadEnabled && p.b2FileName)
      ? await getSignedPhotoUrl(p.b2FileName).catch(()=>null)
      : p.b2FileName ? await getSignedPhotoUrl(p.b2FileName).catch(()=>null) : null,
  })));
