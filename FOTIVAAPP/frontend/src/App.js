...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ minHeight:'100vh', background:'#080808', display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:20, padding:24, fontFamily:'Inter, sans-serif' }}>
          <div style={{ fontSize:48 }}>⚠️</div>
          <div style={{ color:'#fff', fontSize:20, fontWeight:700, textAlign:'center' }}>Algo deu errado</div>
          <div style={{ color:'#666', fontSize:14, textAlign:'center', maxWidth:400 }}>Ocorreu um erro inesperado. Tente recarregar a página.</div>
...
function Private({ children, requireSub = true }) {
  ...
  if (requireSub) {
    const s = user.subscription; const now = new Date();
