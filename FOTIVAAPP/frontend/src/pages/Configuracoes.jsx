          ...
          <div>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ background:'#161616', border:'1px solid rgba(255,255,255,.05)', borderRadius:12, padding:14, marginBottom:14 }}>
                <p style={{ color:'#777', fontSize:13, lineHeight:1.6 }}>📱 Funciona no celular e computador<br/>⏰ 48h, 24h e 2h antes do evento<br/>🔔 Funciona mesmo com o app fechado</p>
              </div>
              ...
              <div style={{ display:'flex', gap:10 }}>
                {(!pushActive || notifStatus !== 'granted') ? (
                  <button onClick={ativarNotificacoes} disabled={notifLoading}
                    ...
                    <Bell size={16}/> {notifLoading ? 'Ativando...' : 'Ativar Notificações Push'}
                  </button>
                ) : (
                  <>
                    <button onClick={testarPush} disabled={notifLoading}
            ...
            <div style={cardStyle}>
              <div style={{ display:'flex', alignItems:'center', gap:14, marginBottom:18 }}>
                <div style={{ width:44, height:44, borderRadius:12, background:'rgba(37,211,102,.1)', border:'1px solid rgba(37,211,102,.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:22 }}>
                  💬
                </div>
                <div>
                  <div style={{ color:'#fff', fontWeight:700, fontSize:15 }}>WhatsApp (CallMeBot)</div>
                  <div style={{ color:'#555', fontSize:13, marginTop:2 }}>Receba alertas direto no seu WhatsApp — grátis</div>
                </div>
              </div>

              {waAtivo && (
                <div style={{ background:'rgba(37,211,102,.08)', border:'1px solid rgba(37,211,102,.2)', borderRadius:10, padding:'9px 14px', marginBottom:14, fontSize:13, color:'#25D366' }}>
                  ✅ WhatsApp conectado — {user?.whatsappPhone || ''}
                </div>
              )}

              <div style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.06)', borderRadius:12, padding:14, marginBottom:14, fontSize:12, color:'#666', lineHeight:1.7 }}>
                <strong style={{ color:'#aaa' }}>Como configurar:</strong><br/>
                1. Adicione <strong style={{ color:'#25D366' }}>+34 644 59 21 83</strong> no WhatsApp<br/>
                2. Mande a mensagem: <code style={{ background:'#1a1a1a', padding:'1px 6px', borderRadius:4, color:'#E87722' }}>I allow callmebot to send me messages</code><br/>
                3. Cole a API Key que ele responder abaixo
              </div>

              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                <div>
                  <label style={{ display:'block', color:'#555', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:.5, marginBottom:5 }}>Seu número WhatsApp (com código do país)</label>
                  <input value={waPhone} onChange={e => setWaPhone(e.target.value)}
                    placeholder="Ex: 5537999990000"
                    style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:9, padding:'10px 14px', color:'#fff', fontSize:14, outline:'none', fontFamily:'inherit' }}/>
                </div>
                <div>
                  <label style={{ display:'block', color:'#555', fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:.5, marginBottom:5 }}>API Key do CallMeBot</label>
                  <input value={waApiKey} onChange={e => setWaApiKey(e.target.value)}
                    placeholder="Ex: 123456"
                    style={{ width:'100%', background:'rgba(255,255,255,.05)', border:'1px solid rgba(255,255,255,.1)', borderRadius:9, padding:'10px 14px', color:'#fff', fontSize:14, outline:'none', fontFamily:'inherit' }}/>
                </div>
                <div style={{ display:'flex', gap:10 }}>
                  <button onClick={salvarWhatsApp} disabled={waLoading || !waPhone || !waApiKey}
                    style={{ flex:1, padding:'11px', borderRadius:10, background:'linear-gradient(135deg,#25D366,#128C7E)', color:'#fff', border:'none', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', opacity:waLoading||!waPhone||!waApiKey?.5:1 }}>
                    {waLoading ? 'Salvando...' : '💾 Salvar'}
                  </button>
                  {waAtivo && (
                    <button onClick={testarWhatsApp} disabled={waLoading}
                      style={{ flex:1, padding:'11px', borderRadius:10, background:'rgba(37,211,102,.1)', color:'#25D366', border:'1px solid rgba(37,211,102,.25)', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
                      📲 Testar WhatsApp
                    </button>
                  )}
                </div>
            ...
            {notifStatus === 'denied' && (
              <div style={{ background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: '#EF4444' }}>
                ❌ Permissão bloqueada. Clique no 🔒 na barra de endereço → Notificações → Permitir
              </div>
            )}
            <div style={{ display: 'flex', gap: 10 }}>
              {(!pushActive || notifStatus !== 'granted') ? (
                ...
                <>
                  <button onClick={testarNotificacao} disabled={notifLoading}
                    style={{ flex: 1, padding: '12px', borderRadius: 10, background: 'rgba(34,197,94,.1)', color: '#22C55E', border: '1px solid rgba(34,197,94,.25)', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                    🔔 Testar notificação
                  </button>
                  <button onClick={desativarNotificacoes} disabled={notifLoading}
                    style={{ flex: 1, padding: '12px', borderRadius: 10, background: 'rgba(239,68,68,.08)', color: '#EF4444', border: '1px solid rgba(239,68,68,.2)', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                    🔕 Desativar
                  </button>
                </>
              )}
        ...
        {tab === 'conta' && (
          <div style={cardStyle}>
            ...
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 8 }}>
              <a href="/assinatura" style={{ display: 'block', width: '100%', padding: '12px', background: 'linear-gradient(135deg,#E87722,#C85A00)', color: '#fff', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: 'pointer', textAlign: 'center', textDecoration: 'none' }}>
                🔄 Ver e trocar planos
              </a>
              {user?.subscription?.status === 'active' && (
                ...
                }} style={{ width: '100%', padding: '10px', background: 'transparent', color: '#666', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
                  ⚙️ Gerenciar assinatura (Stripe)
                </button>
              )}
            </div>
          </div>
