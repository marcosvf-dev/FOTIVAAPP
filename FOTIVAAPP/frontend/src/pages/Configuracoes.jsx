...
export default function Configuracoes() {
  ...
  const [waPhone,  setWaPhone]  = useState(user?.whatsappPhone  || '');
  const [waApiKey, setWaApiKey] = useState(user?.whatsappApiKey || '');
  const [waAtivo,  setWaAtivo]  = useState(!!(user?.whatsappPhone && user?.whatsappApiKey));
  const [waLoading,setWaLoading]= useState(false);

  async function salvarWhatsApp() {
    setWaLoading(true);
    try {
      await api.put('/api/auth/profile', { whatsappPhone: waPhone, whatsappApiKey: waApiKey });
      setWaAtivo(true);
  ...
  async function testarWhatsApp() {
    setWaLoading(true);
    try {
      await api.post('/api/push/test-whatsapp');
